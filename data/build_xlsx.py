...
for key, sc in data["scenarios"].items():
    ...
    if is_module:
        ...
        labels = col_labels + ["Target Inventory (1.5mo)", "Inventory Gap", "Consecutive Surplus Months"]
        for i, label in enumerate(labels, start=1):
            c = sh.cell(row=header_row, column=i, value=label)
            c.font = HEADER_FONT
            c.fill = HEADER_FILL
            c.alignment = Alignment(horizontal="center")
        for idx, (r_i, row) in enumerate(zip(range(header_row + 1, header_row + 1 + len(sc["rows"])), sc["rows"])):
            ...
            f_gap.font = BODY_FONT
            # Consecutive surplus months: if gap>0, previous count+1, else 0
            if idx == 0:
                f_consec = sh.cell(row=r_i, column=10, value=f"=IF(I{r_i}>0,1,0)")
            else:
                f_consec = sh.cell(row=r_i, column=10, value=f"=IF(I{r_i}>0,J{r_i-1}+1,0)")
            f_consec.font = BODY_FONT
        for i in range(1, len(labels) + 1):
            sh.column_dimensions[get_column_letter(i)].width = 20
